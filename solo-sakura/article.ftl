        ...
        <div class="pattern-center single-center">
            ...
            <header class="pattern-header single-header"><h1 class="entry-title">${article.articleTitle}</h1>
                <p class="entry-census"><span><a href="${servePath}/"><img
                            ...
                            class="bull">·</span><span data-uvstaturl="${servePath}${article.articlePermalink}">${article.articleViewCount}</span> 次阅读<span
                            class="bull">·</span><span data-uvstatcmt="${article.oId}">${article.articleCommentCount}</span> ${commentLabel}</p>
                <#if article.articleCategory?? && article.articleCategory != '' && article.categoryURI??>
